        ...
        if( ufunct[0] != '\0' )
        {
          ...
          else
          {
            ...
            *totptr     = psubroutine(pgsub);
            ...
            {
              epicsPrintf("Link %s - Array too large! %ld Bytes\n", fldnames[i], num);
              status = S_db_errArg;
            }
            else
            {
              ...
              *totptr = num;
#if DEBUG
              epicsPrintf("Link(%s): Address = 0x%x, Bytes = %ld\n", fldnames[i], (unsigned int)(*valptr), *totptr );
#endif
            }
          }
        }
        else
        {
          if( *typptr == DBF_STRING )
          {
            ...
            else
            {
              ...
              *totptr = num;
#if DEBUG
              epicsPrintf("Link(%s): Address = 0x%x, Bytes = %d\n", fldnames[i], (unsigned int)(*valptr), *totptr);
#endif
            }
          }
          else
    ...
    if( !status )
    {
      ...
      for( i=0; i<ARG_MAX; i++, plinkin++, typptr++, valptr++, nelptr++ )
      {
        switch( (*plinkin).type ) 
        {
          case (CONSTANT):
#if DEBUG
            epicsPrintf("Input Link %s is a CONSTANT\n", Ifldnames[i] );
#endif
            if( *nelptr < 2 )
            {
          ...
          case (PV_LINK):
#if DEBUG
            epicsPrintf("Input Link %s is a PV_LINK\n", Ifldnames[i] );
#endif
            break;

          case (CA_LINK):
#if DEBUG
            epicsPrintf("Input Link %s is a CA_LINK\n", Ifldnames[i] );
#endif
            break;

          case (DB_LINK):
#if DEBUG
            epicsPrintf("Input Link %s is a DB_LINK\n", Ifldnames[i] );
#endif
            break;
      ...
      for( i=0; i<ARG_MAX; i++, plinkout++, typptr++, valptr++ )
      {
        switch( (*plinkout).type ) 
        {
          case (CONSTANT):
#if DEBUG
            epicsPrintf("Output Link %s is a CONSTANT\n", Ofldnames[i] );
#endif
            break;

          case (PV_LINK):
#if DEBUG
            epicsPrintf("Output Link %s is a PV_LINK\n", Ofldnames[i] );
#endif
            break;

          case (CA_LINK):
#if DEBUG
            epicsPrintf("Output Link %s is a CA_LINK\n", Ofldnames[i] );
#endif
            break;

          case (DB_LINK):
#if DEBUG
            epicsPrintf("Output Link %s is a DB_LINK\n", Ofldnames[i] );
#endif
            break;
      ...
      if( !status )
      {
        if( pgsub->lflg == genSubLFLG_IGNORE )
        {
          if( pgsub->snam[0] != '\0' )
          {
            ...
            ret = symFindByName( sysSymTbl, temp, (void *)&sub_addr, (void *)&sub_type );
#if DEBUG
            epicsPrintf("Calling symFindByName from init_record: %s\n", temp);
#endif
            if( (ret < 0) || ((sub_type & N_TEXT) == 0) )
            {
              recGblRecordError(S_db_BadSub,(void *)pgsub,"genSubRecord(init_record)");
...
if (!pgsub->pact) {
    ...
    if( !status )
    {
      if( pgsub->snam[0] != '\0' )
      {
        if( strcmp(pgsub->snam, pgsub->onam) )  
        {
          ...
          ret = symFindByName( sysSymTbl, temp, (void *)&sub_addr, (void *)&sub_type );
#if DEBUG
          epicsPrintf("Calling symFindByName from process\n");
#endif
          if( (ret < 0) || ((sub_type & N_TEXT) == 0) )
          {
  ...
  if( !status )
  {
    ...
    for( i=0; i<ARG_MAX; i++, plinkin++, valptr++, nelptr++, typptr++ )
    {
      ...
      {
#if DEBUG
        epicsPrintf("Status %ld from dbGetLink (%s)\n", status, Ifldnames[i]);
#endif
        break;
      }
    }
...
/* Do not check this because dbPutLink returns -1 if channel 
   not connected
      if( status )
      {
        epicsPrintf("Status %d from dbPutLink (%s)\n", status, Ofldnames[i]);
        break;
      }
*/
    }
    ...
    else if( pgsub->eflg == genSubEFLG_ON_CHANGE )
    {
      if(monitor_mask)
      {
        ...
        for( i=0; i<ARG_MAX; i++, valptr++, ovlptr++, totptr++ )
        {
          ...
          {
#if DEBUG
            epicsPrintf("Differences in field %s were found. Total Bytes = %d\n", Ofldnames[i], *totptr);
#endif
...
static long do_sub( struct genSubRecord *pgsub )
{
  ...
  if( pgsub->snam[0] != '\0' )
  {
    ...
    else
    {
      ...
      if( status < 0 )
        recGblSetSevr(pgsub, SOFT_ALARM, MAJOR_ALARM);
...
#if DEBUG
  epicsPrintf("Calling cvt_dbaddr...\n");
#endif
  flag  = 1;
  nNew  = 0;
  error = findField( flag, paddr, &no_elements, nNew );
  if( error )
    epicsPrintf("cvt_dbaddr: Could not find field\n");
  return(0);
}
...
#if DEBUG
  epicsPrintf("Calling get_array_info...\n");
#endif
  *offset = 0;
  nNew    = 0;
  flag    = 2;
  error   = findField( flag, paddr, no_elements, nNew );
  if( error )
    epicsPrintf("get_array_info: Could not find field\n");
  return(0);
}
...
#if DEBUG
  epicsPrintf("Calling put_array_info...\n");
#endif
  flag  = 3;
  error = findField( flag, paddr, &no_elements, nNew );
  if( error )
    epicsPrintf("put_array_info: Could not find field\n");
  return(0);
}
  ...
  if( after )
  {
    if( pgsub->lflg == genSubLFLG_IGNORE )
    {
      if( pgsub->snam[0] != '\0' )
      {
        ...
        ret = symFindByName( sysSymTbl, temp, (void *)&sub_addr, (void *)&sub_type );
#if DEBUG
        epicsPrintf("Calling symFindByName from special\n");
#endif
        if( (ret < 0) || ((sub_type & N_TEXT) == 0) )
        {
          recGblRecordError(S_db_BadSub,(void *)pgsub,"genSubRecord(process) symFindByName failed");
