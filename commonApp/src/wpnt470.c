...
*/
	...
	value <<= point;
/*
	Might need to set the interrupt mask here to insure atomic
	read modify write
*/

        temp = input((unsigned)&c_blk->brd_ptr->port[port].b_select);
	temp = temp & ~bpos | value;
        output((unsigned)&c_blk->brd_ptr->port[port].b_select,temp);
/*
	The interrupt mask should be cleared here
*/
	return(0);
}
